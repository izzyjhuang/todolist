...
useEffect(() => {
  const checkMoveTasks = () => {
    const now = new Date();
    if (now.getHours() === 7 && now.getMinutes() === 33) {
      moveTasksToToday();
    }
  };

  const moveInterval = setInterval(checkMoveTasks, 6000); // Check every minute
...
useEffect(() => {
  ...
  const routineInterval = setInterval(checkLoadRoutine, 6000); // Check every minute
